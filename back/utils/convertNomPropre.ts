...
export function convertNomPropre(name:string) : string  {
    ...
    .map(word => word
      .split('-').map(sub => 
        sub.split("'").map((part, i) =>
          i === 0 ? capitalize(part) : part // on ne met en majuscule que le premier morceau avant '
        ).join("'")
      ).join('-')
    )
    .join(' ');
...
export default convertNomPropre
...
export class PrincipalTag {
    ...
    this.tagStyle = `mr-1 pl-2 pr-2 pt-1 pb-1 text-center cursor-pointer m-w-20 m-h-5 rounded-t-lg bg-${color}`;
...
export class SecondaryTag {
    ...
    this.type = type;
    this.title = title;
    this.tagStyle = `mr-2 mb-1 pr-2 pl-2 pb-1 pt-1 text-center cursor-pointer m-h-20 rounded-r-lg bg-${color}`;
...
export const defineActiveTags = (principalTagsList : PrincipalTag[], secondaryTagsList : SecondaryTag[]) => {
    ...
    const startSecondaryTag = localStorage.getItem('secondaryTag') ?? (firstSecondaryTag !== undefined ? firstSecondaryTag : '');
